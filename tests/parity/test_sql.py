...
class TestEmptyDB():
    ...
    def test_create_db(self, empty_db_parity_session):
        base.metadata.create_all(empty_db_parity_session.db_engine)
        pass

    def test_remove_db(self, empty_db_parity_session):
        base.metadata.drop_all(empty_db_parity_session.db_engine)
        pass
...
class TestEmptyTables():
    ...
    def test_verify_block_contents(self,
        ...
        empty_table_parity_session = add_block_number(
                                    block_number=56160,
                                    ether_sql_session=empty_table_parity_session)
        empty_table_parity_session.db_session.commit()

        # comparing values of blocks
        block_properties_in_sql = empty_table_parity_session.db_session.query(Blocks).first().to_dict()
        assert block_properties_in_sql == expected_block_properties

        # comparing values of uncles
        uncle_properties_in_sql = empty_table_parity_session.db_session.query(Uncles).first().to_dict()
        assert uncle_properties_in_sql == expected_uncle_properties

        # comparing values of transactions
        transaction_properties_in_sql = empty_table_parity_session.db_session.query(Transactions).first().to_dict()
        assert transaction_properties_in_sql == expected_transaction_properties

        # comparing values of receipts
        receipt_properties_in_sql = empty_table_parity_session.db_session.query(Receipts).first().to_dict()
    ...
    def test_push_block_range(self, empty_table_parity_session):
        ...
        assert empty_table_parity_session.db_session.query(
                func.max(Blocks.block_number)).scalar() == 46150
        # number of rows in Blocks
        assert empty_table_parity_session.db_session.query(Blocks).count() == 11
        # number of rows in uncles
        assert empty_table_parity_session.db_session.query(Uncles).count() == 1
        # number of rows in transactions
        assert empty_table_parity_session.db_session.query(Transactions).count() == 1
        # number of rows in receipts
        assert empty_table_parity_session.db_session.query(Receipts).count() == 1
        empty_table_parity_session.db_session.close()
        base.metadata.drop_all(empty_table_parity_session.db_engine)
